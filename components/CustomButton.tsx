...
const CustomButton = ({ value }: { value: string }) => {
  return (
    <div className="py-5">
      <Link href="/contact">
        <button className="group relative rounded-2xl backdrop-blur-xl border-2 border-red-500/30 bg-gradient-to-br from-red-900/40 via-black-900/60 to-black shadow-2xl hover:shadow-red-500/30 hover:shadow-2xl hover:scale-[1.02] hover:-translate-y-1 active:scale-95 transition-all duration-500 ease-out cursor-pointer hover:border-red-400/60 overflow-hidden">
          ...
          <div className="relative z-10 flex items-center gap-1 scroll-px-20">
            <div className=" text-center p-4">
              <p className="text-white font-bold text-xl uppercase group-hover:text-red-300 transition-colors duration-300 drop-shadow-sm">
                {value}
              </p>
            </div>
          </div>
